...
xlsx = pd.read_excel('Аналитик данных _Тестовое задание _РБК.xlsx', sheet_name='Выгрузка')
articles = pd.read_excel('Аналитик данных _Тестовое задание _РБК.xlsx', sheet_name='База публикаций')

# process view data: exclude views with URLs containing certain patterns
if 'Адрес страницы' in xlsx.columns:
    valid_views = ~xlsx['Адрес страницы'].str.contains(r'test\.pro|test\.v2\.pro|feature\-rbcnews|\/preview\/|staging\.pro|staging\.v2\.pro')
...
articles_views = articles.merge(xlsx, left_on='URL', right_on='Адрес страницы', how='left')

# create a table of top 20 articles by views with their titles
top20_articles = articles_views.loc[valid_views].groupby(['ID публикации', 'Заголовок материала'])['Просмотры'].sum().nlargest(20)
...
try:
    ...
    top20_articles['Заголовок материала'] = '<a href="' + top20_articles['URL к материалу'] + '">' + top20_articles['Заголовок материала'] + '</a>'
